...
{*****************************************************************************
*  ZLibEx.pas (zlib 1.2.9)                                                   *
*  replacement for original System.ZLib (see notes below)                    *
*                                                                            *
...
*                                                                            *
*  revision history                                                          *
*    2020.05.03  updated to version 1.2.9
*                J. Rathlev (kontakt@rathlev-home.de)
*                32/64 bit version, uses obj files from Delphi libraries
*                changes made for compatibility to ExtFileTools:
*                  TZDecompressionStream.Create, TZDecompressionStream.Read
*                    number of bytes to be decompressed added
*                  TZCompressionStream.Destroy hangs up if
*                    destination drive is full (prob. error in deflate)
*                    check for Z_BUF_ERROR before flushing
*                  gz functions removed
...
type
  ...
  public
    ...
    constructor Create(source: TStream; ALength : int64); overload;
    constructor Create(source: TStream; WindowBits: Integer); overload;
    constructor Create(source: TStream; ALength : int64; WindowBits: Integer); overload;
    destructor Destroy; override;
    function Read(var buffer; count: Longint): Longint; override;
    function Write(const buffer; count: Longint): Longint; override;
    function Read(Buffer: TBytes; Offset, Count: Longint): Longint; override;
...
begin
  ...
  try
    ...
    dr:=deflate(FZStream, Z_FINISH);
    if dr<>Z_BUF_ERROR then begin
      while ZCompressCheck(dr) <> Z_STREAM_END do
      begin
...
constructor TZDecompressionStream.Create(source: TStream; ALength : int64);
begin
  Create(source, ALength, 15);
end;

constructor TZDecompressionStream.Create(source: TStream; WindowBits: Integer);
begin
  Create(source, 0, WindowBits);
end;

constructor TZDecompressionStream.Create(source: TStream; ALength : int64; WindowBits: Integer);
begin
  inherited Create(source);
  FZStream.next_in := @FBuffer[0];
  FZStream.avail_in := 0;
  if ALength<=0 then SLength:=source.Size else SLength:=ALength;
...
begin
  ...
  begin
    if FZStream.avail_in = 0 then begin
      if SLength>Length(FBuffer) then n:=Length(FBuffer) else n:=SLength;
      dec(SLength,n);
      if n>0 then FZStream.avail_in := FStream.Read(FBuffer, n);
